# -*- coding:utf-8 -*-

import ddt
import unittest
import urllib
import urllib.request
import json
import os
import sys, getopt

sys.path.append('..')



from utils.config import Config
from utils.taskdata import TaskData, Task
from utils.logger import LoggerInstance
from utils.parametrizedtestcase import ParametrizedTestCase
from utils.api.websocketapi import WebSocketApi
from utils.api.commonapi import *
from utils.base import WebSocket

time.sleep(2)
print("stop all")
stop_all_nodes()
print("start all")
start_nodes([0,1,2,3,4,5,6], Config.DEFAULT_NODE_ARGS, True, True)
time.sleep(60)
print("waiting for 60s......")

#from ws_api_conf import Conf
from test_config import *

logger = LoggerInstance

wsapi = WebSocketApi()


class test_websocket_1(ParametrizedTestCase):
		
	def setUp(self):
		logger.open( self._testMethodName+".log",self._testMethodName)
		
	def tearDown(self):
		logger.close(self.m_result)
		pass
	
	def test_base_001_heartbeat(self):
		try:
			(process, response) = wsapi.heartbeat()
			ASSERT(process, "")
		except:
			pass

	def test_abnormal_002_heartbeat(self):
		try:
			stop_node(0)
			(process, response) = wsapi.heartbeat()
			start_node(0, Config.DEFAULT_NODE_ARGS)
			time.sleep(5)
			ASSERT(not process, "")
		except:
			pass
	
	def test_abnormal_003_heartbeat(self):
		try:
			ws = WebSocket()
			ws.exec(heartbeat_gap=400)
			process=True
			# (result, response) = wsapi.heartbeat()
			ASSERT(process, "")
		except:
			process=False

	def test_base_004_subscribe(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.subscribe([config.CONTRACT_ADDRESS_CORRECT])
			ASSERT(process, "")
		except:
			pass

	def test_normal_005_subscribe(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.subscribe([config.CONTRACT_ADDRESS_INCORRECT_2])
			ASSERT(process, "")
		except:
			pass
	
	def test_normal_006_subscribe(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.subscribe([config.CONTRACT_ADDRESS_INCORRECT_3])
			ASSERT(process, "")
		except:
			pass

	def test_normal_007_subscribe(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.subscribe([config.CONTRACT_ADDRESS_CORRECT], sevent=True)
			ASSERT(process, "")
		except:
			pass

	def test_normal_008_subscribe(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.subscribe([config.CONTRACT_ADDRESS_CORRECT], sevent=False)
			ASSERT(process, "")
		except:
			pass

	def test_normal_009_subscribe(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.subscribe([config.CONTRACT_ADDRESS_CORRECT], sevent=None)
			ASSERT(process, "")
		except:
			pass

	def test_normal_010_subscribe(self):
		try:
			(process, response) = wsapi.subscribe(None, sevent=True)
			ASSERT(process, "")
		except:
			pass

	def test_normal_011_subscribe(self):
		try:
			(process, response) = wsapi.subscribe(None, sevent=0)
			ASSERT(process, "")
		except:
			pass

	def test_normal_012_subscribe(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.subscribe([config.CONTRACT_ADDRESS_CORRECT], sjsonblock=True)
			ASSERT(process, "")
		except:
			pass

	def test_normal_013_subscribe(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.subscribe([config.CONTRACT_ADDRESS_CORRECT], sjsonblock=False)
			ASSERT(process, "")
		except:
			pass

	def test_normal_014_subscribe(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.subscribe([config.CONTRACT_ADDRESS_CORRECT], sjsonblock=None)
			ASSERT(process, "")
		except:
			pass

	def test_normal_015_subscribe(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.subscribe([config.CONTRACT_ADDRESS_CORRECT], sjsonblock=0)
			ASSERT(process, "")
		except:
			pass

	def test_normal_016_subscribe(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.subscribe([config.CONTRACT_ADDRESS_CORRECT], srawblock=True)
			ASSERT(process, "")
		except:
			pass

	def test_normal_017_subscribe(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.subscribe([config.CONTRACT_ADDRESS_CORRECT], srawblock=False)
			ASSERT(process, "")
		except:
			pass

	def test_normal_018_subscribe(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.subscribe([config.CONTRACT_ADDRESS_CORRECT], srawblock=None)
			ASSERT(process, "")
		except:
			pass

	def test_normal_019_subscribe(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.subscribe([config.CONTRACT_ADDRESS_CORRECT], srawblock=0)
			ASSERT(process, "")
		except:
			pass

	def test_normal_020_subscribe(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.subscribe([config.CONTRACT_ADDRESS_CORRECT], sblocktxhashs=True)
			ASSERT(process, "")
		except:
			pass

	def test_normal_021_subscribe(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.subscribe([config.CONTRACT_ADDRESS_CORRECT], sblocktxhashs=False)
			ASSERT(process, "")
		except:
			pass

	def test_normal_022_subscribe(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.subscribe([config.CONTRACT_ADDRESS_CORRECT], sblocktxhashs=None)
			ASSERT(process, "")
		except:
			pass

	def test_normal_023_subscribe(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.subscribe([config.CONTRACT_ADDRESS_CORRECT], sblocktxhashs=0)
			ASSERT(process, "")
		except:
			pass

	def test_base_024_getgenerateblocktime(self):
		try:
			(process, response) = wsapi.getgenerateblocktime()
			ASSERT(process, "")
		except:
			pass

	def test_normal_025_getgenerateblocktime(self):
		try:
			(process, response) = wsapi.getgenerateblocktime({"height":"1"})
			ASSERT(process, "")
		except:
			pass

	def test_base_026_getconnectioncount(self):
		try:
			(process, response) = wsapi.getconnectioncount()
			ASSERT(process, "")
		except:
			pass

	def test_abnormal_027_getconnectioncount(self):
		try:
			stop_node(0)
			(process, response) = wsapi.getconnectioncount()
			start_node(0, Config.DEFAULT_NODE_ARGS)
			time.sleep(5)
			ASSERT(not process, "")
		except:
			pass

	def test_normal_028_getconnectioncount(self):
		try:
			(process, response) = wsapi.getconnectioncount({"height":"1"})
			ASSERT(process, "")
		except:
			pass

	def test_base_029_getblocktxsbyheight(self):
		try:
			(process, response) = wsapi.getblocktxsbyheight(config.HEIGHT_CORRECT)
			ASSERT(process, "")
		except:
			pass

	def test_normal_031_getblocktxsbyheight(self):
		try:
			(process, response) = wsapi.getblocktxsbyheight(config.HEIGHT_BORDER)
			ASSERT(process, "")
		except:
			pass

	def test_abnormal_032_getblocktxsbyheight(self):
		try:
			(process, response) = wsapi.getblocktxsbyheight(config.HEIGHT_INCORRECT_1)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_033_getblocktxsbyheight(self):
		try:
			(process, response) = wsapi.getblocktxsbyheight(config.HEIGHT_INCORRECT_2)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_034_getblocktxsbyheight(self):
		try:
			(process, response) = wsapi.getblocktxsbyheight(config.HEIGHT_INCORRECT_3)
			ASSERT(not process, "")		
		except:
			pass

	def test_base_035_getblockbyheight(self):
		try:
			(process, response) = wsapi.getblockbyheight(config.HEIGHT_CORRECT)
			ASSERT(process, "")
		except:
			pass

	def test_normal_037_getblockbyheight(self):
		try:
			(process, response) = wsapi.getblockbyheight(config.HEIGHT_BORDER)
			ASSERT(process, "")
		except:
			pass

	def test_abnormal_038_getblockbyheight(self):
		try:
			(process, response) = wsapi.getblockbyheight(config.HEIGHT_INCORRECT_1)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_039_getblockbyheight(self):
		try:
			(process, response) = wsapi.getblockbyheight(config.HEIGHT_INCORRECT_2)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_040_getblockbyheight(self):
		try:
			(process, response) = wsapi.getblockbyheight(config.HEIGHT_INCORRECT_3)
			ASSERT(not process, "")
		except:
			pass

	def test_base_041_getblockbyhash(self):
		try:
			(process, response) = wsapi.getblockbyhash(config.BLOCK_HASH_CORRECT)
			ASSERT(process, "")
		except:
			pass
	
	def test_abnormal_043_getblockbyhash(self):
		try:
			(process, response) = wsapi.getblockbyhash(config.BLOCK_HASH_INCORRECT_1)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_044_getblockbyhash(self):
		try:
			(process, response) = wsapi.getblockbyhash(config.BLOCK_HASH_INCORRECT_2)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_045_getblockbyhash(self):
		try:
			(process, response) = wsapi.getblockbyhash(config.BLOCK_HASH_INCORRECT_3)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_046_getblockbyhash(self):
		try:
			(process, response) = wsapi.getblockbyhash(config.BLOCK_HASH_INCORRECT_4)
			ASSERT(not process, "")
		except:
			pass

	def test_base_047_getblockheight(self):
		try:
			(process, response) = wsapi.getblockheight()
			ASSERT(process, "")
		except:
			pass
	
	def test_normal_048_getblockheight(self):
		try:
			stop_nodes([0, 1, 2, 3, 4, 5, 6])
			start_nodes([0, 1, 2, 3, 4, 5, 6], Config.DEFAULT_NODE_ARGS)
			time.sleep(10)
			(process, response) = wsapi.getblockheight()
			ASSERT(process, "")
		except:
			pass
	
	def test_base_049_getblockhashbyheight(self):
		try:
			(process, response) = wsapi.getblockhashbyheight(config.HEIGHT_CORRECT)
			ASSERT(process, "")
		except:
			pass

	def test_normal_051_getblockhashbyheight(self):
		try:
			(process, response) = wsapi.getblockhashbyheight(config.HEIGHT_BORDER)
			ASSERT(process, "")
		except:
			pass

	def test_abnormal_052_getblockhashbyheight(self):
		try:
			(process, response) = wsapi.getblockhashbyheight(config.HEIGHT_INCORRECT_1)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_053_getblockhashbyheight(self):
		try:
			(process, response) = wsapi.getblockhashbyheight(config.HEIGHT_INCORRECT_2)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_054_getblockhashbyheight(self):
		try:
			(process, response) = wsapi.getblockhashbyheight(config.HEIGHT_INCORRECT_3)
			ASSERT(not process, "")
		except:
			pass

	def test_base_055_gettransaction(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.gettransaction(config.TX_HASH_CORRECT)
			ASSERT(process, "")
		except:
			pass

	def test_abnormal_056_gettransaction(self):
		try:
			(process, response) = wsapi.gettransaction(config.TX_HASH_INCORRECT_2)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_057_gettransaction(self):
		try:
			(process, response) = wsapi.gettransaction(config.TX_HASH_INCORRECT_1)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_058_gettransaction(self):
		try:
			(process, response) = wsapi.gettransaction(config.TX_HASH_INCORRECT_2)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_059_gettransaction(self):
		try:
			(process, response) = wsapi.gettransaction(config.TX_HASH_INCORRECT_3)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_060_gettransaction(self):
		try:
			(process, response) = wsapi.gettransaction(config.TX_HASH_INCORRECT_4)
			ASSERT(not process, "")
		except:
			pass

	def test_base_061_sendrawtransaction(self):
		try:
			(process, response) = wsapi.sendrawtransaction(config.RAW_TRANSACTION_DATA_CORRECT)
			ASSERT(process, "")
		except:
			pass

	def test_abnormal_063_sendrawtransaction(self):
		try:
			(process, response) = wsapi.sendrawtransaction(config.RAW_TRANSACTION_DATA_INCORRECT_2)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_064_sendrawtransaction(self):
		try:
			(process, response) = wsapi.sendrawtransaction(config.RAW_TRANSACTION_DATA_INCORRECT_3)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_065_sendrawtransaction(self):
		try:
			(process, response) = wsapi.sendrawtransaction(config.RAW_TRANSACTION_DATA_INCORRECT_1)
			ASSERT(not process, "")
		except:
			pass

	def test_base_066_get_version(self):
		try:
			task = Task(Config.BASEAPI_PATH + "/ws/getversion.json")
			task.set_type("ws")
			param = None
			if param and isinstance(param, dict):
				taskrequest = task.request()
				for key in param:
					taskrequest[key] = param[key]
				task.set_request(taskrequest)
			(process, response) = run_single_task(task)
			ASSERT(process, "")
		except:
			pass

	def test_normal_068_get_version(self):
		try:
			task = Task(Config.BASEAPI_PATH + "/ws/getversion.json")
			task.set_type("ws")
			param = {"height":""}
			if param and isinstance(param, dict):
				taskrequest = task.request()
				for key in param:
					taskrequest[key] = param[key]
				task.set_request(taskrequest)
			(process, response) = run_single_task(task)
			ASSERT(process, "")
		except:
			pass

	def test_normal_069_get_version(self):
		try:
			task = Task(Config.BASEAPI_PATH + "/ws/getversion.json")
			task.set_type("ws")
			param = {"height":"abc"}
			if param and isinstance(param, dict):
				taskrequest = task.request()
				for key in param:
					taskrequest[key] = param[key]
				task.set_request(taskrequest)
			(process, response) = run_single_task(task)
			ASSERT(process, "")
		except:
			pass

	def test_base_070_getbalancebyaddr(self):
		try:
			(process, response) = wsapi.getbalancebyaddr(config.ACCOUNT_ADDRESS_CORRECT)
			ASSERT(process, "")
		except:
			pass

	def test_abnormal_071_getbalancebyaddr(self):
		try:
			(process, response) = wsapi.getbalancebyaddr(config.ACCOUNT_ADDRESS_INCORRECT_2)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_072_getbalancebyaddr(self):
		try:
			(process, response) = wsapi.getbalancebyaddr(config.ACCOUNT_ADDRESS_INCORRECT_1)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_073_getbalancebyaddr(self):
		try:
			(process, response) = wsapi.getbalancebyaddr(config.ACCOUNT_ADDRESS_INCORRECT_3)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_074_getcontract(self):
		try:
			(process, response) = wsapi.getcontract(config.ACCOUNT_ADDRESS_INCORRECT_4)
			ASSERT(not process, "")
		except:
			pass

	def test_base_075_getcontract(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.getcontract(config.CONTRACT_ADDRESS_CORRECT)
			ASSERT(process, "")
		except:
			pass

	def test_abnormal_077_getcontract(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.getcontract(config.CONTRACT_ADDRESS_INCORRECT_2)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_078_getcontract(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.getcontract(config.CONTRACT_ADDRESS_INCORRECT_1)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_079_getcontract(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.getcontract(config.CONTRACT_ADDRESS_INCORRECT_3)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_080_getcontract(self):
		try:
			time.sleep(5)
			(process, response) = wsapi.getcontract(config.CONTRACT_ADDRESS_INCORRECT_4)
			ASSERT(not process, "")
		except:
			pass

	def test_base_081_getsmartcodeeventbyheight(self):
		try:
			(process, response) = wsapi.getsmartcodeeventbyheight(config.HEIGHT_CORRECT)
			ASSERT(process, "")
		except:
			pass

	def test_normal_082_getsmartcodeeventbyheight(self):
		try:
			(process, response) = wsapi.getsmartcodeeventbyheight(config.HEIGHT_CORRECT)
			ASSERT(process, "")
		except:
			pass

	def test_abnormal_083_getsmartcodeeventbyheight(self):
		try:
			(process, response) = wsapi.getsmartcodeeventbyheight(config.HEIGHT_BORDER)
			ASSERT(process, "")
		except:
			pass

	def test_normal_084_getsmartcodeeventbyheight(self):
		try:
			(process, response) = wsapi.getsmartcodeeventbyheight(config.HEIGHT_INCORRECT_1)
			ASSERT(process, "")
		except:
			pass

	def test_normal_085_getsmartcodeeventbyheight(self):
		try:
			(process, response) = wsapi.getsmartcodeeventbyheight(config.HEIGHT_INCORRECT_2)
			ASSERT(process, "")
		except:
			pass

	def test_abnormal_086_getsmartcodeeventbyheight(self):
		try:
			(process, response) = wsapi.getsmartcodeeventbyheight(config.HEIGHT_INCORRECT_3)
			ASSERT(not process, "")
		except:
			pass

	def test_base_087_getsmartcodeeventbyhash(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.getsmartcodeeventbyhash(config.TX_HASH_CORRECT)
			ASSERT(process, "")
		except:
			pass

	def test_normal_088_getsmartcodeeventbyhash(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.getsmartcodeeventbyhash(config.TX_HASH_INCORRECT_5)
			ASSERT(process, "")
		except:
			pass

	def test_abnormal_089_getsmartcodeeventbyhash(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.getsmartcodeeventbyhash(config.TX_HASH_INCORRECT_1)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_090_getsmartcodeeventbyhash(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.getsmartcodeeventbyhash(config.TX_HASH_INCORRECT_2)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_091_getsmartcodeeventbyhash(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.getsmartcodeeventbyhash(config.TX_HASH_INCORRECT_3)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_092_getsmartcodeeventbyhash(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.getsmartcodeeventbyhash(config.TX_HASH_INCORRECT_4)
			ASSERT(not process, "")
		except:
			pass

	def test_base_093_getblockheightbytxhash(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.getblockheightbytxhash(config.TX_HASH_CORRECT)
			ASSERT(process, "")
		except:
			pass

	def test_normal_094_getblockheightbytxhash(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.getblockheightbytxhash(config.TX_HASH_INCORRECT_5)
			ASSERT(process, "")
		except:
			pass

	def test_abnormal_095_getblockheightbytxhash(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.getblockheightbytxhash(config.TX_HASH_INCORRECT_1)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_096_getblockheightbytxhash(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.getblockheightbytxhash(config.TX_HASH_INCORRECT_2)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_097_getblockheightbytxhash(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.getblockheightbytxhash(config.TX_HASH_INCORRECT_3)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_098_getblockheightbytxhash(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.getblockheightbytxhash(config.TX_HASH_INCORRECT_4)
			ASSERT(not process, "")
		except:
			pass

	def test_base_099_getmerkleproof(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.getmerkleproof(config.TX_HASH_CORRECT)
			ASSERT(process, "")
		except:
			pass
	
	def test_normal_100_getmerkleproof(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.getmerkleproof(config.TX_HASH_INCORRECT_5)
			ASSERT(process, "")
		except:
			pass

	def test_abnormal_101_getmerkleproof(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.getmerkleproof(config.TX_HASH_INCORRECT_1)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_102_getmerkleproof(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.getmerkleproof(config.TX_HASH_INCORRECT_2)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_103_getmerkleproof(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.getmerkleproof(config.TX_HASH_INCORRECT_3)
			ASSERT(not process, "")
		except:
			pass

	def test_abnormal_104_getmerkleproof(self):
		try:
			time.sleep(10)
			(process, response) = wsapi.getmerkleproof(config.TX_HASH_INCORRECT_4)
			ASSERT(not process, "")
		except:
			pass

	def test_base_105_getsessioncount(self):
		try:
			(process, response) = wsapi.getsessioncount()
			ASSERT(process, "")
		except:
			pass

	def test_abnormal_106_getsessioncount(self):
		try:
			stop_node(0)
			(process, response) = wsapi.getsessioncount()
			start_node(0, Config.DEFAULT_NODE_ARGS)
			time.sleep(5)
			ASSERT(not process, "")
		except:
			pass
	
	'''
	def test_107_getstorage(self):
		log_path = "107_getstorage.log"
		task_name = "107_getstorage"
		logger.open(".log","")
		(result, response) = wsapi.getstorage(CONTRACT_ADDRESS_CORRECT, KEY_CORRECT)
		self.abnormal_finish(task_name, log_path, result, "")

	def test_108_getstorage(self):
		log_path = "108_getstorage.log"
		task_name = "108_getstorage"
		logger.open(".log","")
		(result, response) = wsapi.getstorage(CONTRACT_ADDRESS_INCORRECT_2, KEY_CORRECT)
		logger.close(result)

	def test_109_getstorage(self):
		log_path = "109_getstorage.log"
		task_name = "109_getstorage"
		logger.open(".log","")
		(result, response) = wsapi.getstorage(CONTRACT_ADDRESS_INCORRECT_3, KEY_CORRECT)
		self.abnormal_finish(task_name, log_path, result, "")

	def test_110_getstorage(self):
		log_path = "110_getstorage.log"
		task_name = "110_getstorage"
		logger.open(".log","")
		(result, response) = wsapi.getstorage(CONTRACT_ADDRESS_INCORRECT_4, KEY_CORRECT)
		self.abnormal_finish(task_name, log_path, result, "")

	def test_111_getstorage(self):
		log_path = "111_getstorage.log"
		task_name = "111_getstorage"
		logger.open(".log","")
		(result, response) = wsapi.getstorage(CONTRACT_ADDRESS_INCORRECT_1, KEY_CORRECT)
		self.abnormal_finish(task_name, log_path, result, "")

	def test_112_getstorage(self):
		log_path = "112_getstorage.log"
		task_name = "112_getstorage"
		logger.open(".log","")
		(result, response) = wsapi.getstorage(CONTRACT_ADDRESS_CORRECT, KEY_CORRECT)
		logger.close(result)

	def test_113_getstorage(self):
		log_path = "113_getstorage.log"
		task_name = "113_getstorage"
		logger.open(".log","")
		(result, response) = wsapi.getstorage(CONTRACT_ADDRESS_CORRECT, KEY_INCORRECT_2)
		self.abnormal_finish(task_name, log_path, result, "")

	def test_114_getstorage(self):
		log_path = "114_getstorage.log"
		task_name = "114_getstorage"
		logger.open(".log","")
		(result, response) = wsapi.getstorage(CONTRACT_ADDRESS_CORRECT, KEY_INCORRECT_3)
		self.abnormal_finish(task_name, log_path, result, "")

	def test_115_getstorage(self):
		log_path = "115_getstorage.log"
		task_name = "115_getstorage"
		logger.open(".log","")
		(result, response) = wsapi.getstorage(CONTRACT_ADDRESS_CORRECT, KEY_INCORRECT_4)
		self.abnormal_finish(task_name, log_path, result, "")

	def test_116_getstorage(self):
		log_path = "116_getstorage.log"
		task_name = "116_getstorage"
		logger.open(".log","")
		(result, response) = wsapi.getstorage(CONTRACT_ADDRESS_CORRECT, KEY_INCORRECT_1)
		self.abnormal_finish(task_name, log_path, result, "")
	'''
####################################################
if __name__ == '__main__':
	suite = unittest.main()