...
class TestSample1(ParametrizedTestCase):
	def test_main(self):
		...
		try:
			...
			#step 6 user_A_withdraw_role_B
			task5 = Task("tasks/user_A_withdraw_role_B.json")
			(result, response) = call_contract(task5)
			if not result:
				raise Error("user_A_withdraw_role_B error")

			#step 7 user_A_invoke_func_B
			task7 = Task("tasks/user_A_invoke_func_B.json")
